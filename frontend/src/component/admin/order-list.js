...
export default function PromotionList() {
    ...
    const getStatusConfig = (status) => {
        switch (status) {
            case "delivered":
                return { label: "Đã giao hàng", style: "bg-emerald-100 text-emerald-800 border-emerald-200" };
            case "pending":
                return { label: "Chờ xử lý", style: "bg-amber-100 text-amber-800 border-amber-200" };
            case "confirmed":
                return { label: "Đã xác nhận", style: "bg-blue-50 text-blue-700 border-blue-200" };
            case "cancelled":
